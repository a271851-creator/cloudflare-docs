...
stmt = env.DB.prepare("SELECT * FROM Customers WHERE CompanyName = ?").bind(some_variable)
```
</TabItem> </Tabs>
...
stmt = env.DB.prepare("SELECT * FROM Customers WHERE CompanyName = ?")
batch_result = await env.DB.batch(to_js([
...
company_name1 = "Bs Beverages"
company_name2 = "Around the Horn"
stmt = await env.DB.batch(to_js([
    env.DB.prepare("SELECT * FROM Customers WHERE CompanyName = ?").bind(company_name1),
    env.DB.prepare("SELECT * FROM Customers WHERE CompanyName = ?").bind(company_name2)
]))
return Response.json(stmt)
```
</TabItem> </Tabs>
...
```py
return_value = await env.DB.exec('SELECT * FROM Customers WHERE CompanyName = "Bs Beverages"')
```
</TabItem> </Tabs>
...
```py
session = env.DB.withSession("<parameter>")
```
</TabItem> </Tabs>
