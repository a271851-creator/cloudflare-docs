...
class WebSocketServer(DurableObject):
	...
	async def fetch(self, request):
		...
		def on_message(event):
			self.handleWebSocketMessage(server, event.data)
		...
		# If the client closes the connection, the runtime will close the connection too.
		def on_close():
			self.handleConnectionClose(server)
			# Clean up proxies
		...
		# Add the WebSocket connection to the map of active sessions.
		self.sessions[server] = {"id": id}

		return Response(None, status=101, web_socket=client)

	async def handleWebSocketMessage(self, ws, message):
		connection = self.sessions[ws]

		# Reply back with the same message to the connection
		ws.send(f"[Durable Object] message: {message}, from: {connection['id']}, to: the initiating client. Total connections: {len(self.sessions)}")

		# Broadcast the message to all the connections,
		# except the one that sent the message.
		for session in self.sessions.keys():
			if session != ws:
				session.send(f"[Durable Object] message: {message}, from: {connection['id']}, to: all clients except the initiating client. Total connections: {len(self.sessions)}")

		# Broadcast the message to all the connections,
		# including the one that sent the message.
		for session in self.sessions.keys():
			session.send(f"[Durable Object] message: {message}, from: {connection['id']}, to: all clients. Total connections: {len(self.sessions)}")

	async def handleConnectionClose(self, ws):
